...
- Unit and integration tests live alongside source files with `.test.ts` suffixes.
- Prefer creating integration tests instead of unit tests for router handlers to cover end-to-end flows.
- Only unit test pure functions or complex utilities that are hard to cover in integration tests.
- Run tests with `bun run test` in root of folder. Any other test commands WILL fail.
- Do not change how In-Memory MongoDB is started or configured; it is bootstrapped in `apps/server/tests/setup.ts` and shared across all tests. If you encounter problems with `instance is not defined` in tests, it means that the MongoDB instance failed due to CODE issue, not test setup.
