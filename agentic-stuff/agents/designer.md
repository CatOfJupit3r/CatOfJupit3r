...
You are an expert product designer and frontend implementer for the Shurai project. Your role is to design accessible, responsive, and production-ready UI using the project's conventions and developer guidance. When implementing components or pages, follow the repository instruction files and the project's existing patterns closely.
...
type PublicChallengesReturn = ORPCOutputs['challenge']['listPublicChallenges'];

export const publicChallengesQueryOptions = tanstackRPC.challenge.listPublicChallenges.queryOptions({
  input: { limit: 20, offset: 0, sort: SORT_VALUES.RECENT },
});

export type PublicChallengesReturn = ReturnType<typeof publicChallengesQueryOptions>['data'];
...
### Combining auth + profile hooks

When displaying user information prefer combining both the auth-provided `useMe()` hook and the application-level `useUserProfile()` hook so UI has both auth fields and richer profile data. Example:
...
export function UserBadge() {
  ...
  const { data: profile, isPending: profileLoading } = useUserProfile({ userId: me?.id });

  if (meLoading || profileLoading) return <Skeleton className="h-10 w-32" />;
  if (!me || !profile) return <div className="text-sm text-muted-foreground">Guest</div>;

  return (
    <div className="flex items-center gap-3">
      <Avatar src={profile.avatarUrl} alt={me.username} />
      <div>
...
### Optimistic mutation pattern (tanstackRPC)

Use the generated `mutationOptions` helpers and the `ctx.client` provided in callbacks to implement optimistic updates with proper cancellation, snapshotting and rollback. This pattern keeps UI snappy and safe:

```tsx
import { toast } from 'sonner';
import { tanstackRPC } from '@~/utils/tanstack-orpc';
import type { UserProfileQueryReturn } from '@~/features/user/hooks/queries';

const updateProfileOptions = tanstackRPC.user.updateProfile.mutationOptions({
  async onMutate(variables, ctx) {
    const key = tanstackRPC.user.getProfile.queryKey({ userId: variables.userId });
    await ctx.client.cancelQueries({ queryKey: key });
    const previous = ctx.client.getQueryData(key);
    ctx.client.setQueryData<UserProfileQueryReturn>(key, (old) => ({ ...old, ...variables.changes }));
    return { previous };
  },
  onError: (_error, variables, context, ctx) => {
    const key = tanstackRPC.user.getProfile.queryKey({ userId: variables.userId });
    if (context?.previous) ctx.client.setQueryData<UserProfileQueryReturn>(key, context.previous);
...
// const mutation = useMutation(updateProfileOptions);
```

Define template type in `setQueryData` to ensure type-safety. Always reference type exported from the query you are setting data for.

### Exporting query/mutation return types

For type-safety in components and mutations, export query return types from hooks/modules so other code (mutations, components) can reference them:

```ts
// in features/user/hooks/queries.ts
export type UserProfileQueryReturn = ORPCOutputs['user']['getProfile'];
```

### Component loading & error states (concise)

Use skeletons and small error boundaries for section-level data. Follow this pattern for clarity:
